...
public class AutoLockSettingPreference extends Preference implements Preference.OnPreferenceClickListener{
    ...
    private void setupKeyboardTimeout() {
        if (checkTimeoutRule()) {
            ...
            BarGunSettingsActivity.showToast("保存成功！");
        } else {
            BarGunSettingsActivity.showToast("时间间隔大于等于180秒小于等于1200秒，请输入正确的时间间隔");
        }
    ...
    private boolean checkTimeoutRule() {
        if (!BarGunSettingsActivity.isNumeric(etLockKeyboard.getText().toString().trim()))
            return false;
